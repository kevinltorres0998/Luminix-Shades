...
export async function POST(request: Request) {
  ...
  if (!/^\S+@\S+\.\S+$/.test(email) || fullName.length < 2 || password.length < 12 || !/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/\d/.test(password)) return Response.json({ ok: false, error: "Use a valid email and a password of at least 12 characters with uppercase, lowercase and a number." }, { status: 422 });
  ...
  try { await env.DB.prepare("INSERT INTO admin_users(id,email,full_name,password_hash,password_salt,role,created_at) VALUES(?,?,?,?,?,?,?)").bind(id, email, fullName, credentials.hash, credentials.salt, firstUser ? "owner" : "admin", now).run(); }
