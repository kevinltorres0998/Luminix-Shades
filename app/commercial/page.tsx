...
const industries = [
  {name:"Corporate Offices",subtitle:"Privacy • Performance • Focus",why:"Corporate workplaces require balanced daylight, meeting privacy, employee comfort and uninterrupted productivity.",ratings:[["Smart Film",5],["Motorized Screen Roller Shades",5],["Custom Drapery",3],["Cellular Shades",2]],benefits:["Reduce screen glare","Protect confidential meetings","Improve employee wellbeing","Preserve open architecture"],applications:["Executive Offices","Conference Rooms","Reception Areas","Open Workspace"],best:["Smart Film","Motorized Screen Roller Shades"]},
  {name:"Luxury Hotels",subtitle:"Comfort designed around every guest.",why:"Hospitality environments need intuitive privacy, acoustic softness and automated light control that quietly elevate every stay.",ratings:[["Custom Drapery",5],["Blackout Roller Shades",5],["Cellular Shades",4],["Smart Film",3]],benefits:["Create complete sleep comfort","Automate guest-room light","Soften acoustics","Elevate the luxury atmosphere"],applications:["Guest Suites","Hotel Lobbies","Restaurants","Meeting Salons"],best:["Blackout Roller Shades","Full-Height Custom Drapery"]},
  {name:"Restaurants",subtitle:"Atmosphere that evolves throughout the day.",why:"Restaurants need to transition gracefully from bright daytime service to intimate evenings while maintaining guest comfort and architectural warmth.",ratings:[["Custom Drapery",5],["Motorized Screen Roller Shades",5],["Smart Film",3],["Cellular Shades",1]],benefits:["Control changing daylight","Reduce heat and glare","Create evening intimacy","Add material warmth"],applications:["Dining Rooms","Private Dining","Bars & Lounges","Waterfront Terraces"],best:["Ripple Fold Drapery","Motorized Screen Roller Shades"]},
  {name:"Healthcare",subtitle:"Privacy where trust matters most.",why:"Healthcare spaces require dependable patient privacy, hygienic surfaces and access to natural daylight that supports both patients and staff.",ratings:[["Smart Film",5],["Cellular Shades",4],["Motorized Roller Shades",4],["Custom Drapery",2]],benefits:["Protect patient dignity","Support hygienic cleaning","Preserve restorative daylight","Improve staff wellbeing"],applications:["Consultation Rooms","Treatment Areas","Patient Rooms","Medical Offices"],best:["Smart Film","Light-Filtering Cellular Shades"]},
  {name:"Retail",subtitle:"Architecture that strengthens your brand.",why:"Retail environments depend on controlled sunlight, protected merchandise and a storefront experience that reinforces premium brand perception.",ratings:[["Motorized Screen Roller Shades",5],["Smart Film",4],["Custom Drapery",3],["Cellular Shades",1]],benefits:["Protect merchandise from UV","Control storefront glare","Improve product presentation","Strengthen brand experience"],applications:["Luxury Boutiques","Showrooms","Storefronts","Fitting Rooms"],best:["Motorized Screen Roller Shades","Smart Film"]},
  {name:"Multifamily Developments",subtitle:"Comfort at scale.",why:"Multifamily projects need repeatable, durable systems that deliver consistent comfort, energy performance and architectural unity across many residences.",ratings:[["Motorized Roller Shades",5],["Cellular Shades",5],["Custom Drapery",3],["Smart Film",3]],benefits:["Standardize installation","Improve resident comfort","Reduce energy demand","Maintain design consistency"],applications:["Residential Towers","Amenity Spaces","Leasing Centers","Penthouse Units"],best:["Motorized Roller Shades","Cellular Shades"]},
  {name:"Education",subtitle:"Learning environments with better daylight.",why:"Educational environments benefit from glare reduction, visual comfort and optimized daylight that helps students concentrate while lowering energy use.",ratings:[["Motorized Screen Roller Shades",5],["Cellular Shades",4],["Smart Film",4],["Custom Drapery",1]],benefits:["Reduce classroom glare","Support concentration","Optimize natural daylight","Lower cooling costs"],applications:["Classrooms","Libraries","Lecture Halls","Administration"],best:["Motorized Screen Roller Shades","Smart Film"]},
];
...
export default function CommercialPage(){
  ...
  const [industryPanel,setIndustryPanel]=useState<number|null>(null);
  const [industryCanLeft,setIndustryCanLeft]=useState(false);
  const [industryCanRight,setIndustryCanRight]=useState(true);
  const [compactIndustryPanel,setCompactIndustryPanel]=useState(false);
  ...
  useEffect(()=>{
    ...
    if(!track)return;
    const update=()=>{setIndustryCanLeft(track.scrollLeft>4);setIndustryCanRight(track.scrollLeft+track.clientWidth<track.scrollWidth-4)};
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.challengesIndustries} id="industries"><div className={styles.challenges}><span className={styles.kicker}>COMMERCIAL CHALLENGES</span><div>{challenges.map(([title,copy],index)=><article key={title}><i className={styles[`challengeIcon${index+1}`]} aria-hidden="true"/><h3>{title}</h3><p>{copy}</p></article>)}</div></div><div className={styles.industries}><span className={styles.kicker}>INDUSTRIES WE SERVE</span><div className={styles.industryTrack} ref={industryTrackRef} onWheel={(event)=>{if(Math.abs(event.deltaY)>Math.abs(event.deltaX)){event.preventDefault();event.currentTarget.scrollBy({left:event.deltaY,behavior:"smooth"})}}}>{industries.map((item,index)=><button type="button" data-industry-card className={styles.industryCard} key={item.name} onClick={()=>setIndustryPanel(index)} aria-label={`Explore recommendations for ${item.name}`}><Sprite frame={index} label={`${item.name} commercial environment`}/><span>{item.name}</span><small>{item.subtitle}</small></button>)}</div><nav className={styles.industryControls} aria-label="Industry carousel navigation"><button type="button" onClick={()=>moveIndustries(-1)} disabled={!industryCanLeft} aria-label="Previous industries">←</button><button type="button" onClick={()=>moveIndustries(1)} disabled={!industryCanRight} aria-label="Next industries">→</button></nav></div></section>

    <section className={styles.projects} id="projects"><Fade className={styles.projectsIntro}><span className={styles.kicker}>FEATURED COMMERCIAL PROJECTS</span><h2>Spaces designed<br/>to perform.</h2><a href="/#projects">VIEW ALL PROJECTS <b>→</b></a></Fade><div className={styles.projectStrip}>{projects.map(([name,location,frame],index)=><a href="/#projects" key={name}><Sprite frame={frame} label={name}/><b>{String(index+1).padStart(2,"0")}</b><div><h3>{name}</h3><span>{location}</span></div></a>)}</div></section>
    ...
    <section className={styles.finalCta} id="contact"><Image src="/images/commercial-purpose-map.png" alt="Luxury commercial interior at dusk" fill sizes="100vw"/><div/><Fade className={styles.finalCopy}><span className={styles.kicker}>DESIGNED FOR PERFORMANCE</span><h2>Architecture influences<br/>performance.<br/><em>Let&apos;s design yours.</em></h2><p>Create a commercial environment where privacy, comfort, light and perception work together.</p><div className={styles.buttonRow}><a className="button button-gold" href={BOOKING_URL} target="_blank" rel="noopener noreferrer">SCHEDULE A CONSULTATION</a><a className="button button-outline" href="mailto:hello@luminixshades.com?subject=Commercial%20Project%20Quote">REQUEST A QUOTE</a></div></Fade></section>
    <AnimatePresence>{industryPanel!==null&&<><motion.div className={styles.industryScrim} aria-hidden="true" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/><motion.aside className={styles.industryPanel} data-industry-panel role="dialog" aria-modal={compactIndustryPanel} aria-labelledby="industry-panel-title" initial={compactIndustryPanel?{y:"100%"}:{x:"100%"}} animate={compactIndustryPanel?{y:0}:{x:0}} exit={compactIndustryPanel?{y:"100%"}:{x:"100%"}} transition={{duration:.5,ease:[.22,1,.36,1]}}><button type="button" className={styles.panelClose} onClick={()=>setIndustryPanel(null)} aria-label="Close industry details">×</button><AnimatePresence mode="wait" initial={false}><motion.div key={industryPanel} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-10}} transition={{duration:.28}}><span className={styles.kicker}>INDUSTRY STRATEGY</span><h2 id="industry-panel-title">{industries[industryPanel].name}</h2><p className={styles.panelSubtitle}>{industries[industryPanel].subtitle}</p><section><small>WHY THIS ENVIRONMENT</small><p>{industries[industryPanel].why}</p></section><section><small>RECOMMENDED SOLUTIONS</small>{industries[industryPanel].ratings.map(([name,score])=><div className={styles.panelRating} key={name}><Stars score={score as number}/><span>{name}</span></div>)}</section><section><small>WHY WE RECOMMEND THIS</small><ul>{industries[industryPanel].benefits.map(item=><li key={item}>{item}</li>)}</ul></section><section><small>IDEAL APPLICATIONS</small><div className={styles.panelApplications}>{industries[industryPanel].applications.map(item=><span key={item}>{item}</span>)}</div></section><section><small>BEST COMBINATION</small><div className={styles.panelCombination}><strong>{industries[industryPanel].best[0]}</strong><i>+</i><strong>{industries[industryPanel].best[1]}</strong></div></section><a className="button button-gold" href={BOOKING_URL} target="_blank" rel="noopener noreferrer">SCHEDULE CONSULTATION</a><a className={styles.panelLink} href="#projects" onClick={()=>setIndustryPanel(null)}>EXPLORE SIMILAR PROJECTS <b>→</b></a></motion.div></AnimatePresence></motion.aside></>}</AnimatePresence>
    <SiteFooter/>
  </main>;
}
